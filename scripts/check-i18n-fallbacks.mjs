...
// fileURLToPath (not URL.pathname) so Windows file:// URLs don't produce a
// malformed leading-slash path like /C:/... (which cascades into a doubled
// drive letter and ENOENT when joined).
    ...
    const fallbackDocs = listFiles(sourceRoot, (file) => [".md", ".mdx"].includes(extname(file)))
      ...
      .filter(
        (doc) =>
          ...
          // Docusaurus never emits a route for a folder's README.md when an
          // index exists (it's a GitHub-convention dead file), so a fallback
          // page sourced from it can't exist either — skip it.
          basename(doc) !== "README.md"
      );

...
function main() {
  ...
  // Tolerate CRLF (Windows checkouts): split on \r?\n so a trailing \r never
  // ends up embedded in a route and breaks the set comparison.
  const baseline = readFileSync(BASELINE_FILE, "utf8").split(/\r?\n/).filter(Boolean).sort();

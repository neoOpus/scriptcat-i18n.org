...
سكربتات الخلفية مناسبة للسكربتات التي تحتاج إلى الاستمرار في التشغيل بشكل دائم. سكربتات الخلفية هي نوع سكربت خاص بـ ScriptCat؛ تعمل في بيئة معزولة (sandbox) ولا يمكنها الوصول إلى الـ DOM. يمكن تطويرها باستخدام نفس واجهات GM APIs الخاصة بـ Tampermonkey، ويتم توضيح ملاحظات التوافق في الوثائق.

## سكربت الخلفية (`@background`) {#background-script-background}

يتم التصريح عن سكربت الخلفية بسمة `@background`. يتيح للسكربت الاستمرار في العمل في الخلفية بعد تفعيل السكربت أو بدء تشغيل المتصفح.

## السكربت المجدول (`@crontab`) {#scheduled-script-crontab}

> السكربت المجدول هو نوع من سكربتات الخلفية مناسب للمهام التي تحتاج إلى **التشغيل المتكرر على دورة زمنية**.

يتم التصريح عن السكربت المجدول بسمة `@crontab`. يدعم الجدولة على مستوى الدقائق والثواني، ويوفر الصيغة الموسعة الخاصة بـ ScriptCat `once` / `once(...)` لتجنب التشغيل أكثر من مرة ضمن نفس الدورة الزمنية.

⚠️ ملاحظات:

* في السكربت الواحد، **يؤثر `@crontab` الأول فقط**
* يُنصح ألا يتجاوز **وقت التنفيذ الفردي + وقت إعادة المحاولة** للسكربت فترة cron، وإلا فقد تتداخل عمليات التنفيذ
...
حتى لو كانت نقاط زمنية لاحقة ضمن نفس الدورة لا تزال تطابق قاعدة cron، فلن يتم تشغيل السكربت مرة أخرى.
...
* سكربتات فحص الحالة والمزامنة والتقارير
...
* السكربتات التي قد يتجاوز وقت تنفيذها فترة cron بشكل كبير
...
في صفحة قائمة السكربتات، مرر مؤشر الماوس فوق **عمود حالة التشغيل** لرؤية **وقت التنفيذ المجدول التالي** للسكربت.

## السجلات

في صفحة قائمة السكربتات، يؤدي تمرير مؤشر الماوس فوق `عمود حالة التشغيل` إلى إظهار تلميح بحالة تشغيل السكربت؛
...
يمكن تصحيح أخطاء سكربتات الخلفية مباشرة من صفحة محرر السكربت، لكن هذا له القيود التالية:
...
النمط التالي موصى به بشدة، لأنه يسمح أيضاً لمدير السكربتات بمراقبة تنفيذ السكربت.
إذا قام السكربت بأي عملية غير متزامنة، فيجب عليه **إرجاع `Promise`**.
...
يرجى التأكد من استدعاء `resolve` / `reject` فقط بعد انتهاء منطق السكربت فعلياً.
بمجرد استدعائه، يعتبر المدير أن تنفيذ السكربت قد اكتمل، ولن تسري أي عمليات GM لاحقة.

## إعادة المحاولة عند الخطأ

تدعم سكربتات الخلفية في ScriptCat إعادة المحاولة عند الخطأ.
عند فشل السكربت، يمكنه استدعاء `reject` مع `CATRetryError` لتفعيل إعادة المحاولة.

* الحد الأدنى لفاصل إعادة المحاولة: 5 ثوانٍ
* تجنب التعارض مع وقت تنفيذ السكربت نفسه، وإلا فقد يحدث تنفيذ مكرر

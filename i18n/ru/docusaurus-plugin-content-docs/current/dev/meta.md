...
Информация об авторских правах скрипта

### tag

Теги скрипта, разделённые запятыми или пробелами

### compatible

Информация о совместимости, отображаемая на GreasyFork

### scriptUrl

URL пользовательского скрипта, на который ссылается подписочный скрипт

### unwrap

Позволяет пользовательскому скрипту обойти sandbox-обёртку и внедряться и выполняться напрямую в нативной глобальной области страницы. Скрипт может напрямую обращаться к настоящим глобальным переменным страницы и изменять их, но не сможет использовать привилегированные API пользовательских скриптов, такие как `GM.*`. Обычно используется в сценариях, требующих глубокого взаимодействия с нативными скриптами страницы, или при переносе существующего обычного скрипта страницы.

### cloudCat

Помечает скрипт как экспортируемый в облачный пакет CloudCat (только SC)

### cloudServer

Облачный сервис CloudCat, используемый скриптом

### exportValue

Значения хранилища скрипта для экспорта при экспорте в облачный скрипт

### exportCookie

Куки для экспорта при экспорте в облачный скрипт

...
> Код всегда выполняется в **MAIN world** страницы (общий `window` с собственным JS страницы), поэтому можно напрямую вызывать функции страницы и читать её переменные — но по той же причине **нет доступа к blob URL расширения** (например, к `blob:` URL, созданному вами через `URL.createObjectURL()` из `Blob`, возвращаемого `CAT.agent.opfs.read` в режиме `"blob"`), так как blob URL ограничен источником расширения. Для работы с blob URL в изолированном окружении используйте SkillScript (см. [Разработка Skill](../skill-dev)).

```javascript
// Вызов JS-функции страницы / чтение переменной страницы
const data = await CAT.agent.dom.executeScript(
  "return window.__APP_STATE__"
);

// Чтение содержимого DOM
...
> `addedNodes` дедуплицируется по ID узла и ограничен 50 записями; узлы, которые были удалены со страницы или невидимы, автоматически пропускаются. `text` — это чистый текст, извлечённый из `outerHTML` узла, обрезанный до 300 символов.

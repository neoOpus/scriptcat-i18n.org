...
Создаёт blob-URL из объекта Blob. ScriptCat управляет жизненным циклом URL.
...
Загружает blob-URL и возвращает данные Blob. Вспомогательная функция для потоковых ответов `GM_xmlhttpRequest`.
...
Загружает URL и разбирает его как Document (в контексте страницы контента, если доступно).
...
Регистрирует пункт меню с полем ввода, позволяя пользователю ввести значение. Callback получает введённое пользователем значение.
